...
for ii =1:2:10
    ...
    ssl = Pre_{ii};  ssr = Pre_{ii+1};
    aal= strsplit(ssl); aar= strsplit(ssr);
    file_name = (char(aal{1}));
    ...
    if length(aal)>3
        ...
        MDL = str2num(char(aal{3}));
        MDR = str2num(char(aar{3}));
        %[ii MDL MDR]
        enl = char(aal{79});
        sens_L = [str2num(char(aal{4:78}))' str2num(enl(1:end-1))];
        
        enr = char(aar{79});
        sens_R = [str2num(char(aar{4:78}))' str2num(enr(1:end-1))];
         plot_30d2_IVF(sens_L,sens_R,MDL,MDR,age((ii+1)/2),file_name)
    end
...
print(fullfile('..\','Data',filename),'-djpeg','-r300');
